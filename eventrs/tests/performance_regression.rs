...
fn test_batch_processing_performance() {
    ...
    let events: Vec<SimpleEvent> = (0..batch_size)
        .map(|i| SimpleEvent { value: i })
        .collect();
...
fn test_priority_handling_performance() {
    ...
    for priority in [Priority::High, Priority::Normal, Priority::Low] {
        ...
        bus.on_with_priority(move |event: SimpleEvent| {
        ...
        .map(|thread_id| {
            ...
            std::thread::spawn(move || {
                for i in 0..events_per_thread {
                    let event = SimpleEvent { value: (thread_id * events_per_thread) + i };
