...
impl AsyncEventBus {
    ...
    pub async fn emit<E: Event>(&self, event: E) -> EventBusResult<()> {
        ...
        if self.config.use_priority_ordering {
            self.process_handlers_with_priority(&event, handlers).await?;
        } else {
    ...
    pub async fn on<E: Event, H: AsyncHandler<E>>(&mut self, handler: H) -> HandlerId {
        self.register_handler(handler, self.config.default_handler_priority).await
    }
    ...
    /// # }
    /// ```
    pub async fn on_with_priority<E: Event, H: AsyncHandler<E>>(&mut self, handler: H, priority: Priority) -> HandlerId {
    ...
    async fn register_handler<E: Event, H: AsyncHandler<E>>(&mut self, handler: H, priority: Priority) -> HandlerId {
    ...
    async fn process_handlers_with_priority<E: Event>(&self, event: &E, handlers: Vec<AsyncHandlerEntry<E>>) -> EventBusResult<()> {
    ...
    async fn process_handlers_sequential<E: Event>(&self, event: &E, handlers: Vec<AsyncHandlerEntry<E>>) -> EventBusResult<()> {
        // Only use concurrent execution if we have enough handlers to make it worthwhile
        if self.config.concurrent_execution && handlers.len() >= self.config.concurrent_threshold {
...
mod tests {
    ...
    async fn test_async_handler_registration() {
        ...
        let handler_id = bus.on(|_event: AsyncTestEvent| async {
    ...
    async fn test_async_event_emission() {
        ...
        bus.emit(AsyncTestEvent { value: 42 }).await.expect("Emit should succeed");
    ...
    async fn test_priority_ordering_async() {
        ...
        bus.on_with_priority(move |_event: AsyncTestEvent| {
