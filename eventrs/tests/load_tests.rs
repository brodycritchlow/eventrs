...
fn test_high_volume_processing() {
    ...
    println!("Starting high-volume load test with {} events across {} threads", total_events, thread_count);
    ...
    println!("  Events/sec: {:.2}", processed as f64 / duration.as_secs_f64());
    
    // Assertions
    assert_eq!(errors, 0, "No errors should occur during high-volume processing");
    ...
    assert!(events_per_second > 1000.0, "Processing rate too low: {:.2} events/sec", events_per_second);
}
...
fn test_sustained_load() {
    ...
    println!("Starting sustained load test for {:?} with {} producer threads", test_duration, thread_count);
    ...
    let monitor_handle = {
        ...
        thread::spawn(move || {
            ...
            while running_clone.load(Ordering::Relaxed) {
                ...
                println!("  Events processed: {}, Rate: {:.2} events/sec", current_count, rate);
    ...
    println!("  Events processed: {}", processed);
    println!("  Average rate: {:.2} events/sec", processed as f64 / actual_duration.as_secs_f64());
    
    // All produced events should be processed
    assert_eq!(processed, total_produced, "All produced events should be processed");
    
    // Should maintain reasonable processing rate
    let avg_rate = processed as f64 / actual_duration.as_secs_f64();
    assert!(avg_rate > 100.0, "Average processing rate too low: {:.2} events/sec", avg_rate);
}
...
fn test_memory_stability() {
    ...
    println!("Starting memory stability test with {} batches of {} events", batch_count, events_per_batch);
    ...
    for batch in 0..batch_count {
        ...
        if batch % 10 == 0 {
            println!("  Batch {}: {} events processed, batch took {:?}", 
    ...
    println!("  Average rate: {:.2} events/sec", final_processed as f64 / total_duration.as_secs_f64());
    
    assert_eq!(final_processed, total_events, "All events should be processed");
}
...
fn test_concurrent_handler_registration() {
    ...
    println!("Starting concurrent handler registration test with {} handlers", handler_count);
    ...
    let total_processed: usize = counts.iter()
    ...
    assert_eq!(registered, handler_count, "All handlers should register successfully");
...
fn test_error_resilience() {
    ...
    println!("Starting error resilience test with {} events (10% error rate)", total_events);
    ...
    assert!(successes > 0, "Some events should be processed successfully");
    assert_eq!(successes + errors, total_events, "All events should reach handlers");
    ...
    assert!(events_per_second > 1000.0, "Processing should remain fast despite errors");
}
...
fn test_scalability() {
    ...
    for &event_count in &load_levels {
        ...
        println!("  Load level {}: {:.2} events/sec", event_count, events_per_second);
