...
mod tests {
    use crate::filter::{
        AnyEvent, ContentAwareCacheFilter, DynamicFilter, EventTypeFilter, EventTypeFilterMode,
    };
    use crate::prelude::*;
    ...
    fn test_dynamic_filter_basic() {
        ...
        let dynamic_filter = DynamicFilter::new_shared("dynamic_test", |_event| true);
    ...
    fn test_cacheable_filter() {
        ...
        let cacheable_filter = DynamicFilter::new_cacheable("cacheable_test", move |event| {
    ...
    fn test_content_aware_cache_filter() {
        ...
        let content_filter = ContentAwareCacheFilter::new("content_aware", move |event| {
    ...
    fn test_dynamic_filter_runtime_update() {
        ...
        let filter = DynamicFilter::new_shared("updatable", |event| {
            if let Some(test_event) = event.as_any().downcast_ref::<TestEvent>() {
                test_event.value < 5
            } else {
                true
