...
impl Filter<SecurityEvent> for ThreatLevelFilter {
    ...
    fn description(&self) -> String {
        format!(
            "Filter: Threat level >= {}, blocked: {}",
            self.min_level, self.only_blocked
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Event 1 (admin, low severity): {}",
        admin_or_critical.evaluate(&event1)
    );
    println!(
        "Event 2 (non-admin, high severity): {}",
        admin_or_critical.evaluate(&event2)
    );
    println!(
        "Event 3 (non-admin, low severity): {}",
        admin_or_critical.evaluate(&event3)
    );
    println!();
    ...
    let security_chain = FilterChain::all()
        .add_filter(ThreatLevelFilter {
            min_level: 5,
            only_blocked: false,
        })
        .add_filter(PredicateFilter::new(
            "internal_source",
            |event: &SecurityEvent| event.source_ip.starts_with("192.168."),
        ))
    ...
    println!(
        "Security Event 1 (high threat, internal): {}",
        security_chain.evaluate(&security_event1)
    );
    println!(
        "Security Event 2 (low threat, internal): {}",
        security_chain.evaluate(&security_event2)
    );
    println!(
        "Security Event 3 (high threat, external): {}",
        security_chain.evaluate(&security_event3)
    );
    println!();
    ...
    println!(
        "Non-admin filter description: {}",
        non_admin_filter.description()
    );

    println!(
        "Admin event passes non-admin filter: {}",
        non_admin_filter.evaluate(&event1)
    );
    println!(
        "Non-admin event passes non-admin filter: {}",
        non_admin_filter.evaluate(&event2)
    );
    println!();
    ...
    println!(
        "Test event passes ANY chain: {}",
        any_chain.evaluate(&test_event)
    );
    ...
    println!(
        "Test event passes ALL chain: {}",
        all_chain.evaluate(&test_event)
    );
    println!();
    ...
    let expensive_filter =
        PredicateFilter::new_expensive("complex_analysis", |event: &UserActionEvent| {
    ...
    println!(
        "Long action by non-admin passes optimized chain: {}",
    ...
    println!(
        "Any event passes allow-all: {}",
        allow_all.evaluate(&test_event)
    );
    println!(
        "Any event passes reject-all: {}",
        reject_all.evaluate(&test_event)
    );
    println!();
    ...
    let dynamic_filter = if user_is_admin {
        ...
        FilterChain::all().add_filter(AllowAllFilter::<UserActionEvent>::new())
    } else {
        // Regular users only see events below certain severity
        FilterChain::all().add_filter(SeverityFilter(min_severity).not())
    };
