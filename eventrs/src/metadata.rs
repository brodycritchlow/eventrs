...
impl EventTransformation {
    ...
    pub fn new<T: Into<String>, A: Into<String>>(transformation_type: T, applied_by: A) -> Self {
        Self {
...
mod tests {
    ...
    fn test_empty_metadata() {
        ...
        let metadata_with_data = EventMetadata::new().with_source("test");
    ...
    fn test_enhanced_source() {
        ...
        let metadata = EventMetadata::new().with_enhanced_source(source.clone());

        assert_eq!(metadata.enhanced_source().unwrap().name, "user_service");
        assert_eq!(
            metadata.enhanced_source().unwrap().source_type,
            SourceType::Service
        );
        assert_eq!(
            metadata.enhanced_source().unwrap().full_path(),
            "platform/auth/user_service"
        );
        assert_eq!(
            metadata.enhanced_source().unwrap().version,
            Some("1.2.3".to_string())
        );
        assert_eq!(
            metadata.enhanced_source().unwrap().instance_id,
    ...
    fn test_event_signature() {
        ...
        let metadata = EventMetadata::new().with_signature(signature);
    ...
    fn test_event_provenance() {
        ...
        let mut derived_metadata = EventMetadata::new().with_provenance(derived_provenance);

        assert!(!derived_metadata.provenance().is_original());
        assert_eq!(derived_metadata.provenance().derivation_depth, 1);
        assert_eq!(
            derived_metadata.provenance().original_event_id,
            Some("original_event_123".to_string())
        );
        ...
        assert_eq!(
            derived_metadata.provenance().transformations[0].transformation_type,
            "filter"
        );
    }
    ...
    fn test_source_types_and_display() {
        ...
        assert_eq!(
            format!("{}", SourceType::Custom("webhook".to_string())),
            "webhook"
        );
    ...
    fn test_event_transformation() {
        ...
        assert_eq!(
            transformation.parameters.get("fields_added"),
            Some(&"user_segment,geo_location".to_string())
        );
    ...
    fn test_metadata_with_all_enhanced_features() {
        ...
        let signature =
            EventSignature::new("ED25519", "signature_bytes_here").with_key_id("payment_key_v2");
