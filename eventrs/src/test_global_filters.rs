...
mod tests {
    use crate::prelude::*;
    use crate::filter::{PredicateAnyFilter, AllowAllAnyFilter, RejectAllAnyFilter, EventTypeFilter, EventTypeFilterMode};
    ...
    fn test_global_filter_basic() {
        ...
        let positive_filter = PredicateAnyFilter::new(
            "positive_values",
            |event| {
        ...
        // Emit events
        bus.emit(TestEvent { value: 5, message: "positive".to_string() }).unwrap();
        bus.emit(TestEvent { value: -3, message: "negative".to_string() }).unwrap();
        bus.emit(TestEvent { value: 10, message: "positive".to_string() }).unwrap();
    ...
    fn test_global_filter_with_priority() {
        ...
        let low_priority_filter = PredicateAnyFilter::new(
            "allow_all",
            |_event| true
        );
        bus.add_global_filter_with_priority("allow_all", Box::new(low_priority_filter), 1);
        
        let high_priority_filter = PredicateAnyFilter::new(
            "block_negative",
            |event| {
                if let Some(test_event) = event.as_any().downcast_ref::<TestEvent>() {
                    test_event.value >= 0
    ...
    fn test_event_type_filter() {
        ...
        bus.emit(TestEvent { value: 42, message: "test".to_string() }).unwrap();
        bus.emit(AdminEvent { user_id: 123, action: "login".to_string() }).unwrap();
        bus.emit(TestEvent { value: 99, message: "test2".to_string() }).unwrap();
    ...
    fn test_allow_all_any_filter() {
        ...
        // Emit events
        bus.emit(TestEvent { value: 1, message: "test".to_string() }).unwrap();
        bus.emit(TestEvent { value: 2, message: "test".to_string() }).unwrap();
    ...
    fn test_global_filtering_enabled_disabled() {
        ...
        bus.emit(TestEvent { value: 3, message: "test".to_string() }).unwrap();
