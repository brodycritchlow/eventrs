...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let filter = PredicateFilter::new("high_value", |event: &TestEvent| event.value > 50);
    ...
    assert_eq!(
        ...
        "Wrong number of events processed"
    );
    ...
    let important_filter =
        PredicateFilter::new("important_only", |event: &TestEvent| event.is_important);
    ...
    bus2.on_filtered(
        move |event: TestEvent| {
            ...
            println!(
                ...
                event.value, event.is_important
            );
        },
    ...
    let high_value_filter =
        PredicateFilter::new("high_value", |event: &TestEvent| event.value > 50);
    ...
    bus2.on_filtered(
        move |event: TestEvent| {
            ...
            println!(
                "  🌟 Important AND high value: {} (important: {}, value: {})",
    ...
    let expected_both = test_events
        .iter()
        .filter(|e| e.is_important && e.value > 50)
        .count();

    assert_eq!(
        final_important, expected_important as u32,
        "Wrong important count"
    );
    assert_eq!(
        final_high_value, expected_high_value as u32,
        "Wrong high value count"
    );
    assert_eq!(
        final_both, expected_both as u32,
        "Wrong both condition count"
    );
    ...
    let high_priority_filter =
        PredicateFilter::new("very_high_value", |event: &TestEvent| event.value > 70);

    bus3.on_filtered_with_priority(
        move |event: TestEvent| {
            order_clone1
                .lock()
                .unwrap()
                .push(format!("HIGH_PRIORITY_{}", event.value));
            println!("  🔥 HIGH PRIORITY processed: {}", event.value);
        },
        Arc::new(high_priority_filter),
        Priority::High,
    );

    // Normal priority handler - for all values > 50
    let order_clone2 = Arc::clone(&execution_order);
    let normal_priority_filter =
        PredicateFilter::new("high_value", |event: &TestEvent| event.value > 50);
    ...
    bus3.on_filtered_with_priority(
        move |event: TestEvent| {
            order_clone2
                .lock()
                .unwrap()
                .push(format!("NORMAL_PRIORITY_{}", event.value));
            println!("  ⭐ NORMAL PRIORITY processed: {}", event.value);
        },
        Arc::new(normal_priority_filter),
        Priority::Normal,
    );
    ...
    assert_eq!(
        final_allow, 3,
        "Allow-all filter should have processed all 3 events"
    );
    assert_eq!(
        final_reject, 0,
        "Reject-all filter should have processed 0 events"
    );
